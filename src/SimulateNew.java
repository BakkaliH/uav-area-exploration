...
public class SimulateNew {
	public static void main(String[] args) {
		...
		GridNew grid = new GridNew(gridWidth, (int)partition, noOfDrones, 1);
		DroneNew[] drones = new DroneNew[noOfDrones];
		...
		for (int d = 0; d < noOfDrones; d++) {
			...
			drones[d] = new DroneNew(d + 1, grid, d + 1, 0.75, 2, newPoint);
			startPoints.add(newPoint);
		}

		long startTime = System.currentTimeMillis();
		grid.start();
		for (DroneNew drone : drones) {
			drone.startDrone();
		}
	...
	private static void displayResults(GridNew grid, DroneNew[] drones, long startTime) {
	...
	private static void prepareJson(GridNew grid, DroneNew[] drones, long startTime) {
